...
def compute_pherintensity(pheromone_map, X, Y, pa):
    if (X >= 1) and (Y >= 1):
        pheromone_inpoint = (pheromone_map[X-pa][Y+pa] + pheromone_map[X][Y+pa] + pheromone_map[X+pa][Y+pa] + pheromone_map[X-pa][Y] + pheromone_map[X][Y] + pheromone_map[X+pa][Y] + pheromone_map[X-pa][Y-pa] + pheromone_map[X][Y-pa] + pheromone_map[X+pa][Y-pa])/9    
    else: pheromone_inpoint = 0 
    #Возвращаем количество феромона в точке
    return pheromone_inpoint
#Выбираем возможное действие с максимальным AQ-значением в зависимости
#от эпсилон
...
def main(): 
        ...
        #награду
        env = StarCraft2Env(map_name="3mvs3zstone", reward_only_positive=False, reward_scale_rate=200, difficulty="1")
        ...
        steps_train = 4  #после начала обучения продолжаем обучать через каждые четыре шага 
        #Размер мини-выборки
        batch_size = 32 
            ...
            while not terminated: 
                ...
                for agent_id in range(n_agents):
                    ...
                    cv2.line(map_data, (int(unit.pos.x), int(unit.pos.y)),
                             (int(unit.pos.x), int(unit.pos.y)), (0, 0,
                              pheromone_map[int(unit.pos.x)][int(unit.pos.y)]), 1)
                ...
                for agent_id in range(n_agents):
                    ...
                    #Обрабатываем исключение при ошибке в действиях 
                ...
                cv2.imshow('DPIQL pheromone map', resized)
                cv2.waitKey(1)
                ...
                for agent_id in range(n_agents):
                    ...
                    #агента
                    exp_buf_L[agent_id].expbufVar.append([obs_agentXY[agent_id],
                    ...
                    if (global_step+1) % copy_steps == 0 and global_step > start_steps:
                        ...
                        (aq_network_list[agent_id].state_dict())
        ...
        plt.figure(num=None, figsize=(6, 3), dpi=150, facecolor='w',  edgecolor='k')
        plt.plot(m_loss)
        plt.xlabel('Номер каждой 1000-й итерации')
